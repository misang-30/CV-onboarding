...
from model import ResNet18FineTun
...
def train_type(modeltype: int ,train_loader :DataLoader,val_loader : DataLoader, hyperparameter : Dict[str, Any], csv_path : str = "training_log.csv", wandbOn: bool =True) :
    # =============== 모델 세팅 ====================

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # GPU 세팅
    if modeltype == 0 :
        model = SmallCNN(width=hyperparameter["width"], dropout_p=hyperparameter["dropout"]) # 모델 생성
        model_name = "SmallCNN"
        ...
        model = ResNet18FineTun(num_classes=10)
        model_name = "ResNet18FineTun"
    else :
        raise ValueError(f"Unknown modeltype option: {modeltype}")

    # =============== wandb 세팅 ====================

    run_name = model_name + "_" + get_name(hyperparameter)
